...
Simulation_Case3 <- function(sim) { 
  ...
  for (omega in c(0,0.5)){
    ...
    for (meth in 1:2){
      ...
      best.lam<-CV.tweedie(X=Data.trainX,y=trainY,m=10,k=5,rho=rho,alpha=alpha,g=g,b0.int=0.3,b.int=c(rep(c(1,0,0,-1,0,0),3),rep(0,6)))$best.lam
      
      test.Fit<-irlsbmd(X=Data.testX,y=testY,v=1/nrow(Data.testX),w=sqrt(g),tau=alpha,rho=rho,g=g,lam=best.lam,b0=0.3,b=c(rep(c(1,0,0,-1,0,0),3),rep(0,6))) 
      
      Beta<-c(test.Fit$b0,test.Fit$b)
      
      beta.coef<-ifelse(test.Fit$b==0,0,1)
      C.coef<-sum(beta.coef[1+(1:6-1)*3]==1)
      IC.coef<-sum(beta.coef[-(1+(1:6-1)*3)]==1)
      
      beta.block<-unlist(lapply(1:8,function(i) sum(beta.coef[(i-1)*3+1:3])))
      
      C.block<-sum(beta.block[1:6]>=1)
      IC.block<-sum(beta.block[7:8]>=1)
    ...
    for (alpha in alpha_v){
      Fit.meth3<-CV.tweedie(X=Data.trainX,y=trainY,m=10,k=5,rho=rho,alpha=alpha,g=g,b0.int=0.3,b.int=c(rep(c(1,0,0,-1,0,0),3),rep(0,6)))
  ...
  write.csv(RESULTS,paste("Case3_",sim,".csv",sep="")) 
  return(RESULTS)
}
...
library(parallel)
n.sim<-50
res <- mclapply(seq(n.sim),Simulation_Case3,mc.cores = getOption("mc.cores", 50L))
Output<-Reduce("+", res)/length(res) 
write.csv(Output,"Sim_case3.csv")

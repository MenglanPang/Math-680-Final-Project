...
Simulation_Case2 <- function(sim) { 
  ...
  write.csv(RESULTS,paste("Case2_",sim,".csv",sep=""))  
  return(RESULTS)
}
...
library(parallel)
n.sim<-50
res <- mclapply(seq(n.sim),Simulation_Case2,mc.cores = getOption("mc.cores", 50L))
Output<-Reduce("+", res)/length(res) 
write.csv(Output,"Sim_case2.csv")
